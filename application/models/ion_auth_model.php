...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function username_check($username = '', $user_id = null)
	{
	    if (empty($username))
	    {
		return FALSE;
        ...
        if ($user_id) { $where = "id != '$user_id'"; } else { $where = "id != '-1'"; }
	 ...
	 * @author Mathew
	 **/
	public function email_check($email = '', $user_id = null)
	{
	    if (empty($email))
	    {
		return FALSE;
	 ...
	 **/
     ...
     /* and updated by rjr!  whee! now it saves new users and updates existing ones. */

	public function save_user($user_id, $username, $password, $email, $additional_data = false, $group_name = false)
	{
	    if ($this->identity_column == 'email' && $this->email_check($email,$user_id))
	    {
		$this->ion_auth->set_error('account_creation_duplicate_email');
		return FALSE;
	    }
	    elseif ($this->identity_column == 'username' && $this->username_check($username,$user_id))
	    {
	    ...
	    {
		for($i = 0; $this->username_check($username,$user_id); $i++)
		{
		    if($i > 0)
		    {
			$username .= $i;
